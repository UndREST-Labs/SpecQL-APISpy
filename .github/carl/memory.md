...
- **CodeQL javascript-all version**: Must be `~0.9.0` (pinned to 0.9.4
  in the lock file). Newer 2.x versions contain syntax that CodeQL
  2.20.2 cannot parse.
...
- **qlpack.yml dependency**: `codeql/javascript-all: ~0.9.0` — do not
  widen or replace with `*`.
