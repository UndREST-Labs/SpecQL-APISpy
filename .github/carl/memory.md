...
- **CodeQL javascript-all version**: Must be exactly 0.9.4; pinned by `JAVASCRIPT_ALL_VERSION="0.9.4"` in `setup.sh` (install-time) and `codeql/javascript-all: "0.9.4"` in `queries/azure-security/qlpack.yml` (runtime dependency). Newer 2.x libraries contain syntax that CodeQL 2.20.2 cannot parse.
